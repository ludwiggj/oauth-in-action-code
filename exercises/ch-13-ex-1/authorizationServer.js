		...
		if (code) {
			...
			if (code.request.client_id == clientId) {
				...
				if (__.contains(code.scope, 'openid') && code.user) {
					var header = { alg: rsaKey.alg, typ: 'JWT', kid: rsaKey.kid };
					var ipayload = {
						iss: "http://localhost:9001",
						sub: code.user.sub,
						aud: client.client_id,
						iat: Math.floor(Date.now() / 1000),
						exp: Math.floor(Date.now() / 1000) + (5 * 60) // 5 minute expiration
					};

					// Add nonce to response if we received it
					if (code.request.nonce) {
						ipayload.nonce = code.request.nonce;
					}

					// Sign the ID token with the RSA key and serialise as a JWT
					var privateKey = jose.KEYUTIL.getKey(rsaKey);
					var id_token = jose.jws.JWS.sign(header.alg, JSON.stringify(header), JSON.stringify(ipayload), privateKey);

					token_response.id_token = id_token;
					console.log('Issued ID token %s', id_token);
				}
